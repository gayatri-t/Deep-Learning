...
with open(i2w_pickle_file, 'rb') as f:
    i2w_data = pickle.load(f)
...
class DecoderNet(nn.Module):
    def __init__(self, hidden_size, output_size, vocab_size, word_dim, dropout_percentage=0.3):
        ...
        self.output_size = len(i2w_data) + 4
        self.vocab_size = len(i2w_data) + 4
        self.word_dim = 1024
        self.embedding = nn.Embedding(len(i2w_data) + 4, 1024)
    ...
    def forward(self, encoder_last_hidden_state, encoder_output, targets=None, mode='train', tr_steps=None):
        ...
        for i in range(seq_len - 1):
            ...
            if random.uniform(0.05, 0.995) > threshold:
                ...
                logits = self.to_final_output(lstm_output.squeeze(1))
                probabilities = F.softmax(logits, dim=1)
                current_input_word = torch.multinomial(probabilities, 1).squeeze(1)
    ...
    def infer(self, encoder_last_hidden_state, encoder_output):
        ...
        decoder_c = torch.zeros(decoder_chs.size())  # Initialize decoder_c
        seq = []
        seq_predictions = []
